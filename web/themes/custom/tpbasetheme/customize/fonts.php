<?php
function tp_basetheme_font_size(){
    $size = array();
    $size[''] = '-- Default --';
    for ($i=12; $i <= 50 ; $i = $i+1) {
        $size[$i] = $i;
...
function tp_basetheme_render_option_font(){
    $fonts = tp_basetheme_fonts();
    $output = '';
...
function tp_basetheme_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function tp_basetheme_typography_enqueue_google_font($font) {

    if($font && $font != "---"){
        if(array_search($font, array_keys(tp_basetheme_fonts())) > 17){
...
function tp_basetheme_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode($json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= tp_basetheme_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= tp_basetheme_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

function tp_basetheme_options_patterns(){
    $output = '';
    $file_path = drupal_get_path('theme', 'tp_basetheme');
