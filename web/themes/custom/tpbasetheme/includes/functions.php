<?php
function tp_basetheme_render_block($key) {
...
function tp_basetheme_makeid($length = 5){
...
function tp_basetheme_base_url(){
  global $base_url;
  $theme_path = drupal_get_path('theme', 'tp_basetheme');
  return $base_url . '/' . $theme_path . '/';
}

function tp_basetheme_preprocess_node(&$variables) {
  ...
  if ($variables['node']->getType() == 'article') {
    ...
    $variables['theme_uri'] = base_path() . drupal_get_path('theme', 'tp_basetheme');
  }
}

function tp_basetheme_preprocess_node__listing(&$variables) {
  $node = $variables['node'];
  $listing_video = '';
  ...
  if(in_array($view_mode, $allowed_view_modes)) {
    ...
    tp_basetheme_add_regions_to_node($allowed_regions, $variables);
  }
}

function tp_basetheme_preprocess_user(&$variables) {
  $allowed_regions = ['listings_user'];
  tp_basetheme_add_regions_to_node($allowed_regions, $variables);
}


function tp_basetheme_preprocess_node__portfolio(&$variables){
  $node = $variables['node'];
...
function tp_basetheme_preprocess_node__event(&$variables){
  $node = $variables['node'];
  $event_date = array();
...
function tp_basetheme_preprocess_breadcrumb(&$variables){
...
*/
function tp_basetheme_add_regions_to_node($allowed_regions, &$variables) {
